...
//!
//! **The question any current measurement turns on.** A quiescent-current figure is worthless with a
//! probe attached, because the debug power request holds the part awake — so the only way a
//! debug-driven harness can serve that case at all is to set the part going, detach, and measure.
//! Whether that is possible is a property of what a detach does, and it had been recorded as an
...
//! code does not zero, or something written to flash. Neither existed in the image this was first
//! run against. Measured 2026-09-01: phase 1 climbs at the rate the application advances it, and
//! phase 3 reads zero.
//!
//! # Usage
//!
//! `does_it_run_detached <elf> <counter-symbol> [detached-seconds]`
//!
//! The counter is any exported symbol the application advances on its own — a loop count, a tick, a
//! sequence number. **It is an argument rather than a constant** because this crate knows nothing
//! about any particular firmware, and a symbol name baked in here would be one firmware's name.
...
fn main() -> anyhow::Result<()> {
    ...
    let counter = args.next().expect("a counter symbol the application advances");
